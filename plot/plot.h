...
//======================================================================
void drawOutOfRangeArrows(TPad* pad, TH1* h,
                          double textSize=0.02,
                          const char* textFormat="%.2f",
                          double arrowSize=0.015,
                          double arrowLength=0.05)

{
  pad->Draw(); // Make sure the ymax of the pad is up-to-date
  ...
  for(int i=1; i<h->GetNbinsX()+1; ++i){
    ...
    if(y>ymax){
      ...
      TArrow* arr=new TArrow(binCenter, ymax-arrowLength*yrange, binCenter, ymax, arrowSize, "|>");
      arr->SetLineColor(h->GetLineColor());
      arr->SetFillColor(h->GetLineColor());
      arr->SetLineWidth(h->GetLineWidth());
      arr->Draw();
      TLatex* la=new TLatex(binCenter, ymax-(arrowLength+0.01)*yrange, TString::Format(textFormat, y));
      la->SetTextSize(textSize);
      la->SetTextFont(h->GetXaxis()->GetLabelFont());
      la->SetTextAlign(23); // Center horiz, top-align vert
      la->SetTextColor(h->GetLineColor());
      la->Draw();
    }
  }
}
