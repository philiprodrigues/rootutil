...
public:
  ...
  */
  ...
  virtual int GetInt(const string& branchName, int ientry, int leafVal=0)
  {
    return (int)GetValue(branchName, ientry, leafVal);
  }

  virtual bool GetBool(const string& branchName, int ientry, int leafVal=0)
  {
    return (bool)GetValue(branchName, ientry, leafVal);
  }

  template<class T>
  T* GetValueArray(const string& branchName, int ientry)
  {
  ...
  template<class T>
  vector<T> GetValueVector(const string& branchName, int ientry)
  {
    GetValue(branchName, ientry);
    LeafAndBranch lb=leavesAndBranches.find(branchName)->second;
